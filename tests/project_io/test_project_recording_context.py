...
def test_registration_rechecks_review_after_waiting_for_concurrent_manifest_writer(
    ...
    attempted = Event()
    transaction_context = project_module.project_manifest_transaction

    def observe_transaction(path):
        attempted.set()
        return transaction_context(path)

    monkeypatch.setattr(project_module, "project_manifest_transaction", observe_transaction)
    with ThreadPoolExecutor(max_workers=1) as pool:
        with transaction_context(project.manifest_path) as transaction:
            pending = pool.submit(
                project.append_registered_inputs,
                participants={}, recordings={"P01__follicular": visit}, **options,
            )
            assert attempted.wait(5), "Registration did not coordinate with the manifest writer"
            updated = json.loads(project.manifest_path.read_bytes())
            updated["tools"]["unrelated"]["concurrent_update"] = True
            transaction.write(updated)
        with pytest.raises(ValueError, match="changed after registration review"):
            pending.result(timeout=5)

    assert json.loads(project.manifest_path.read_bytes()) == updated
    assert project.__dict__ == before
