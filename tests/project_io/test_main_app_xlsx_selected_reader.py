"""Exact behavior tests for the shared Main App XLSX reader."""
...
import Main_App
import Main_App.io.xlsx_selected_reader as selected_reader
...
def test_main_app_does_not_import_stats_xlsx_reader_adapter() -> None:
    main_app_root = Path(Main_App.__file__).parent
    offenders: list[str] = []
    for module_path in main_app_root.rglob("*.py"):
        tree = ast.parse(module_path.read_text(encoding="utf-8"))
        if any(
            isinstance(node, ast.ImportFrom)
            and node.module == "Tools.Stats.io.xlsx_selected_reader"
            for node in ast.walk(tree)
        ):
            offenders.append(module_path.relative_to(main_app_root).as_posix())
