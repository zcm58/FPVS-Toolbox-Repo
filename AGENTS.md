...
- The Stats default group-level significant-harmonics method is locked behavior. It detects significant non-base oddball harmonics from grand-averaged `FullFFT Amplitude (uV)` spectra over the union of predefined ROI electrodes, using strict z > 1.64 against neighboring-bin noise, then includes all non-base oddball harmonics through the highest detected significant harmonic in Summed BCA. Neighboring noise is +/-10 FFT bins around the target after excluding target-1, target, target+1, then dropping the single minimum and single maximum finite amplitude values before computing mean and population SD. Do not change this threshold, default electrode scope, summation rule, noise-window, min/max exclusion, exact-column requirement, or uniform application across participants, conditions, and ROIs unless the user explicitly requests a statistical-method change.
