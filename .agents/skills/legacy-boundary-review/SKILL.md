...
3. Identify whether the requested behavior crosses a retired-path, historical-boundary, or removed-feature path:
...
6. Do not recreate `Legacy_App`; if historical behavior is needed, use existing current-app APIs or a focused adapter outside the retired path.
7. Preserve behavior, processing order, data formats, exports, and user workflows from historical code.
...
- Confirm retired `Legacy_App` paths were not recreated.

## Response Requirements

- State which protected paths were evaluated.
- State whether retired `Legacy_App` paths were affected and why the processing pipeline is unchanged.
