...
So if df is 0.00833 Hz, then bin k=144 corresponds exactly to 144 × 0.00833 ≈ **1.2 Hz**.
...
Equivalently:
- **f_oddball × T** must be an integer  
- or **f_oddball = k × df** for some integer bin k
...
- oddball cycles in the segment = **f_oddball × (N/fs)** is an integer

For f_oddball = 6/5, that means we need:

- (6 × N) / (5 × fs) is an integer
...
- **N = floor(available_samples / step) × step**
...
- **f_bin_hz**: actual bin frequency at k0 (k0 × df)
- **crop_mode**:
