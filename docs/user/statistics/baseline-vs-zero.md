# Baseline vs Zero Tests (Condition x ROI)

Use this page when you need to test whether a condition response is above the no-response baseline.
...
In other words, it tests whether the average response for each **Condition x ROI** is **reliably greater than the "no-response" baseline**.
...
For each Condition x ROI cell, the Toolbox performs a **one-sample t-test** comparing participant-level values against 0:
...
The test is run on **participant-level values** (one value per participant per Condition x ROI).
It does **not** treat electrodes or trials as independent samples.

## Multiple comparisons correction (BH-FDR)
Because multiple Condition x ROI tests are run in the same analysis, the Toolbox applies multiple comparisons correction using **Benjamini-Hochberg False Discovery Rate (BH-FDR)**.
...
- The correction scope is reported (e.g., "global" means all Condition x ROI tests in the output were corrected together as one family).

## How to interpret the output
If a Condition x ROI row has **reject = TRUE** (after BH-FDR correction), you can say:
...
- **Missingness:** If some participants are missing a condition, the test for that Condition x ROI will have a smaller N. The output reports N per test.
