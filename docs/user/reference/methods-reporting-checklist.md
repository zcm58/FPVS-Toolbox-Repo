...
uses MNE to estimate an L2 minimum-norm source power spectrum. The current
method identifier is `l2_mne_hauk_source_psd_v1`.
...
The normal source build is intentionally EEG-only. It uses the Toolbox
BioSemi64 channel geometry with the `fsaverage` template rather than individual
MRI/coregistration, MEG, or EEG/MEG fusion. The inverse is MNE-native L2-MNE
with `method="MNE"`, `loose=0.2`, `depth=None`, `fixed=False`, and
`lambda2=1/9`; it does not apply dSPM, sLORETA, or eLORETA normalization.
...
1. loads the significant oddball harmonics already selected and saved for the
   project during post-processing;
...
3. calls MNE's `mne.minimum_norm.compute_source_psd` on the complete
   repetition-averaged Raw time series, using `n_fft=N`, zero overlap, the Hann
   setting, and the L2-MNE inverse;
...
The current source calculation consumes the saved harmonic-selection record
...
not serve as fallback inputs for the current source-PSD workflow. A normal
rebuild creates the time-domain L2-MNE source-PSD result only; it does not
rebuild eLORETA. A separately validated time-domain eLORETA route, MEG fusion,
and the phase-preserving complex-Fourier Option 2 remain possible later
additions, not current behavior.

## Manuscript Or Preregistration Checklist

Report at least:

- the FPVS Toolbox release or commit and method identifier
  `l2_mne_hauk_source_psd_v1`;
...
- that the saved project significant-harmonic selection was reused, with the
  exact selected harmonic frequencies;
...
- the MNE version, source-space spacing, inverse settings, ad-hoc diagonal EEG
  noise covariance, and source-PSD settings;
...
- participant exclusions, flagged-participant policy, group summary, cluster
  inference, ROI, and lateralization settings used; and
