...
- `src/Main_App/PySide6_App/Backend/`: active preprocessing implementation and processing coordination implementations. Project model, project manager, preprocessing settings, and metadata modules are temporary compatibility wrappers for `Main_App.projects`; `loader.py` is only a compatibility wrapper for the shared BDF loader.
