...
- `src/Main_App/gui/`: canonical import surface for main-window, settings-panel, menu, sidebar, icon, style-token, and update-manager GUI modules. `project_workflows.py` owns project open/create/load/save GUI orchestration, and `post_export_workflows.py` owns GUI-side post-processing export completion handling; remaining modules delegate to existing PySide6 GUI implementations during the package-layout migration.
