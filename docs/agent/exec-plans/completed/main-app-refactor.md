...
- Folder retirement is complete and archived at `docs/agent/exec-plans/completed/main-app-folder-retirement.md`.

## PR Contract

- Keep code changes surgical and behavior-preserving unless the task explicitly changes behavior.
- Read `ARCHITECTURE.md`, `docs/agent/agent-index.md`, and this plan before non-trivial `Main_App` refactors.
- Update `ARCHITECTURE.md`, the relevant `docs/agent/architecture/*.md`, or the nearest scoped `AGENTS.md` when structure, ownership, boundaries, workflows, or verification expectations change.
...
1. Harness setup
   ...
   - Link execution plans from `ARCHITECTURE.md`, `AGENTS.md`, and `docs/agent/agent-index.md`.
...
4. Completion
   ...
   - Move this file to `docs/agent/exec-plans/completed/` when the tracked refactor work is done.
   - Status: complete.

## Verification

- `python scripts/agent_audit.py`
- Relevant skill-local audit scripts from `docs/agent/agent-index.md`
...
- Added and updated the folder-retirement execution plan with inventory categories for every tracked `src/Main_App/Legacy_App/` and `src/Main_App/PySide6_App/` file. That plan is now archived at `docs/agent/exec-plans/completed/main-app-folder-retirement.md`.
...
- Documentation-first requirement: `docs/agent/architecture/post-processing-export-contract.md` records the pre-refactor export contract, output naming, sheet names, column order, metric behavior, and preservation constraints.
...
- Documentation-first requirement: `docs/agent/architecture/preprocessing-contract.md` records the active PySide6 preprocessing owner, fixed pipeline order, preservation rules, and focused verification commands.
...
- Target direction recorded: `docs/agent/architecture/main-app-target-layout.md` defines the long-term purpose-based `Main_App` package shape without permanent `Legacy_App`/`PySide6_App` distinctions.
...
- Documentation-first requirement: `docs/agent/architecture/processing-mixin-contract.md` records the current `ProcessingMixin` host contract, processing order, queue message behavior, finalization behavior, and preservation rules.
...
- Documentation-first requirement: `docs/agent/architecture/eeg-loading-contract.md` records the BDF-only contract, memmap path shape, reference/stim resolution, EXG typing, `standard_1005` 10-10 coverage, `on_missing="warn"`, logging, and return semantics.
...
- Documentation-first requirement: `docs/agent/architecture/project-io.md` records PySide6 file-selection behavior and the rule that user-facing warnings/errors use PySide6-safe message helpers.
...
- Documentation-first requirement: `docs/agent/architecture/post-processing-export-contract.md` now records `build_fft_neighbors_rows` and `write_results_workbook` behavior, including FFT-neighbor metadata, workbook formatting, freeze panes, column sizing, and optional sheet creation.
...
- Documentation-first requirement: `docs/agent/architecture/fft-crop-method.md` records the pre-refactor FFT crop method, constants, fallback reasons, warnings, result fields, and preservation constraints.
...
Latest PySide6_App GUI shell-helper slice:
...
- Next refactor direction: move another small GUI presentation group, likely `sidebar.py` plus any direct shell imports, with focused layout/settings smoke tests. Status: complete.

Latest PySide6_App sidebar slice:
...
- Updated `ARCHITECTURE.md`, `AGENTS.md`, focused architecture docs, `docs/agent/architecture/legacy-quarantine-audit.md`, and the folder-retirement execution plan, now archived at `docs/agent/exec-plans/completed/main-app-folder-retirement.md`.
