...
- The four v1 profile IDs and algorithms are locked:
  ...
  - `fixed_preregistered_domain` accepts an exact Hz list, an upper oddball-
    ...
    that statement. New projects seed this alternative as upper oddball-
    harmonic index 6 so its default follows the project rate; missing-mode
    legacy projects retain the historical exact-frequency list.
  ...
  - `dzhelyova_poncet_two_consecutive_failures` calculates condition-specific
    ...
    overlap neither enters the sum nor counts as a failure. Reaching the end of
    the project filter/Nyquist-backed technical domain before the failure pair
    is a hard error; there is no user BCA ceiling to raise.
...
- Adaptive local-Z math remains locked across profiles. Candidate spacing comes
  from the immutable project oddball rate. The candidate domain is the common
  intersection of processing-owned spectral eligibility across included
  workbooks; there is no live 16.8-Hz, 40-Hz, or BCA-ceiling path. For each
  ...
  do not add a nearest-bin workaround. A target or required noise bin inside an
  effective notch is unavailable. Adaptive profiles skip that hole without
  counting it as a detection or failure; fixed/preregistered profiles preserve
  the declaration and fail rather than silently shrinking it.
...
- Managed project participant-condition exclusions are applied by the shared
  ...
  by the exact project-oddball-rate bin index (the integer analyzed oddball-
  cycle count), and flags
...
- After the canonical harmonic list is selected, the `BCA (uV)` sheet must also
  contain every exact included project-harmonic column formatted as
  `{frequency:.4f}_Hz`. Do not use
