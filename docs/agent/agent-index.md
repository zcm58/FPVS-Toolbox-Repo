...
- The completed v2.1 processing workflow is recorded in `docs/agent/exec-plans/completed/v2-1-processing-workflow.md`; read it before changing project setup, manifests, raw-file discovery, single-file or batch processing, Excel output layout, processing ledger behavior, Plot Generator group handling, or Stats group metadata.
- The completed filter-before-downsample behavior change is recorded in `docs/agent/exec-plans/completed/preprocessing-filter-downsample-order.md`; read it before changing preprocessing order, preprocessing fingerprints, filter duration scaling, or locked processing-order tests.
