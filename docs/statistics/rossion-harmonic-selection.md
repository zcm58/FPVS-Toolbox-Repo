...
Use this page when you need to understand or report Rossion-style harmonic selection.
...
- It creates a single, interpretable number per
  **participant x condition x ROI** that can be used in RM-ANOVA, mixed
...
a group mean Z for each ROI x harmonic. Within each ROI, harmonics were
...
Summed BCA values were then computed for each subject x condition x ROI
by extracting baseline-corrected amplitude (BCA; uV) at the selected
