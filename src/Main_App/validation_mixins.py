...
class ValidationMixin:
    def _validate_inputs(self):
        self.debug("DEBUG_VALIDATE: _validate_inputs START")
        self.debug("DEBUG_VALIDATE: _validate_inputs called via self.log.")
        ...
        if not self.data_paths:
            ...
            self.debug("DEBUG_VALIDATE: Returning False - No data_paths")
            return False
        self.debug("DEBUG_VALIDATE: Data paths validated.")
        ...
        if not save_folder:
            ...
            self.debug("DEBUG_VALIDATE: Returning False - No save_folder")
            return False
        if not os.path.isdir(save_folder):
            try:
                ...
                self.debug(f"DEBUG_VALIDATE: Returning False - Cannot create save_folder: {e}")
                return False
        self.debug("DEBUG_VALIDATE: Save folder validated.")
        ...
        try:
            ...
            self.debug("DEBUG_VALIDATE: Basic parameters and thresholds validated.")
            ...
            self.debug(f"DEBUG_VALIDATE: Returning False - Invalid parameter (AssertionError/ValueError): {e}")
            return False
            ...
            self.debug(f"DEBUG_VALIDATE: Returning False - General parameter validation error: {e_gen}")
            return False

        self.debug("DEBUG_VALIDATE: Proceeding to Event Map validation.")
        event_map = {}
        try:
            ...
            for i, entry_widgets in enumerate(self.event_map_entries):
                ...
                if not label_widget or not id_widget:
                    ...
                    self.debug(f"DEBUG_VALIDATE: Returning False - Event map row {i+1} malformed.")
                    return False
                ...
                if not lbl_str and not id_str:
                    if len(self.event_map_entries) == 1:
                        ...
                        self.debug("DEBUG_VALIDATE: Returning False - Only event map row is empty.")
                        return False
                    self.debug(f"DEBUG_VALIDATE: Row {i+1} is completely empty, skipping.")
                    continue
                ...
                if not lbl_str:
                    ...
                    label_widget.focus_set()
                    self.debug(f"DEBUG_VALIDATE: Returning False - Event map row {i+1} no label.")
                    return False
                if not id_str:
                    ...
                    id_widget.focus_set()
                    self.debug(f"DEBUG_VALIDATE: Returning False - Event map row {i+1} no ID for label '{lbl_str}'.")
                    return False
                ...
                if lbl_str in labels_seen:
                    ...
                    label_widget.focus_set()
                    self.debug(f"DEBUG_VALIDATE: Returning False - Duplicate label '{lbl_str}'.")
                    return False
                ...
                try:
                    ...
                    id_widget.focus_set()
                    self.debug(f"DEBUG_VALIDATE: Returning False - Invalid ID for '{lbl_str}': '{id_str}'.")
                    return False

                event_map[lbl_str] = num_id
                self.debug(f"DEBUG_VALIDATE: Added to event_map: '{lbl_str}' -> {num_id}")
            ...
            if not event_map:  # If after iterating all rows, event_map is still empty
                ...
                self.debug("DEBUG_VALIDATE: Returning False - event_map is empty after loop.")
                return False

            params['event_id_map'] = event_map
            self.validated_params = params
            self.debug("DEBUG_VALIDATE: Event Map validated successfully.")
            ...
            self.debug(f"DEBUG_VALIDATE: Returning False - Unexpected Event Map error: {e_map_general}")
            return False

        self.log("Inputs Validated Successfully.")
        self.log(f"Effective Parameters for this run: {self.validated_params}")
        self.debug("DEBUG_VALIDATE: _validate_inputs RETURNING TRUE")
        return True
