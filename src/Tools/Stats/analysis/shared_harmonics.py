...
from Tools.Stats.analysis.group_harmonics import _build_harmonic_domain
...
def _resolve_z_sheet_name(file_path: str) -> str | None:
    """Handle the resolve z sheet name step for the Stats workflow."""
...
def _find_first_z_columns_with_fallback(
    ...
    """Handle the find first z columns with fallback step for the Stats workflow."""
    for pid in subjects:
...
class SharedHarmonicsResult:
    """Represent the SharedHarmonicsResult part of the Stats tool."""
...
def _select_two_consecutive_significant(
    ...
    *,
    ...
    """Handle the select two consecutive significant step for the Stats workflow."""
    selected: list[float] = []
    sig_run = 0
    nonsig_run = 0
    criterion_met = False
...
def compute_shared_harmonics(
    *,
    ...
    """Handle the compute shared harmonics step for the Stats workflow."""
    if not conditions:
...
def intersect_condition_harmonics(
    *,
    ...
    """Handle the intersect condition harmonics step for the Stats workflow."""
    harmonics_by_roi: dict[str, list[float]] = {}
...
def export_shared_harmonics_summary(
    *,
    ...
    """Handle the export shared harmonics summary step for the Stats workflow."""
