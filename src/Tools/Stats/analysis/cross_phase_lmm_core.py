"""Generic cross-phase LMM utilities for Stats.
...
def _run_backup_2x2(df: pd.DataFrame, logger: logging.Logger) -> Dict[str, object]:
    ...
    def _welch_t_from_stats(
        ...
        """Run the welch t from stats helper used by the Stats workflow."""
    ...
    def _paired_t(values_a: np.ndarray, values_b: np.ndarray) -> Dict[str, float]:
        """Run the paired t helper used by the Stats workflow."""
...
def _build_fixed_effects_table(result) -> List[Dict[str, object]]:
    """Run the build fixed effects table helper used by the Stats workflow."""
...
def _design_matrix_for_scenario(
    ...
    """Run the design matrix for scenario helper used by the Stats workflow."""
    try:
        import patsy
...
def _build_contrast(
    ...
    """Run the build contrast helper used by the Stats workflow."""
    ...
    def _add_contrast(label: str, vec: np.ndarray):
        """Run the add contrast helper used by the Stats workflow."""
...
def run_cross_phase_lmm(
    ...
    def _effect_label_from_test(test: Dict[str, object]) -> str | None:
        """Run the effect label from test helper used by the Stats workflow."""
        ttype = test.get("type")
    ...
    def _append_backup_rows(backup: Dict[str, object] | None) -> None:
        """Run the append backup rows helper used by the Stats workflow."""
        if not backup:
            return
