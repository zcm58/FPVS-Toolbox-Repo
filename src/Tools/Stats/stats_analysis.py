...
def prepare_all_subject_summed_bca_data(subjects, conditions, subject_data, base_freq, log_func):
    all_subject_data = {}
...
def run_paired_tests(all_subject_data, subjects, cond_a, cond_b, rois_to_analyze, roi_label, log_func):
    ...
    for roi_name in rois_to_analyze:
        ...
        values_a, values_b = [], []
        for pid in subjects:
            val_a = all_subject_data.get(pid, {}).get(cond_a, {}).get(roi_name, np.nan)
            val_b = all_subject_data.get(pid, {}).get(cond_b, {}).get(roi_name, np.nan)
            if not (pd.isna(val_a) or pd.isna(val_b)):
                values_a.append(val_a)
                values_b.append(val_b)
        n_pairs = len(values_a)
        if n_pairs < 3:
            output_lines.append(f"  Insufficient paired data (N={n_pairs}). Minimum 3 pairs required for a t-test.\n")
            continue
        try:
            ...
            is_sig = p_value_raw < alpha
            p_value_str = "< .0001" if p_value_raw < 0.0001 else f"{p_value_raw:.4f}"
            significant_overall |= is_sig
            if is_sig:
                output_lines.append("  FINDING: SIGNIFICANT DIFFERENCE found between conditions.")
            else:
                output_lines.append("  FINDING: NO SIGNIFICANT DIFFERENCE found between conditions.")
            ...
            output_lines.append(f"  Error performing t-test for this ROI: {e}\n")
            log_func(f"Paired t-test error for ROI {roi_name} ({cond_a} vs {cond_b}): {e}")
    if not results:
