...
def log_rm_anova_p_minima(anova_df: pd.DataFrame) -> None:
    ...
    def _series_metrics(column: str) -> tuple[float | None, bool]:
        """Handle the series metrics step for the Stats workflow."""
    ...
    *,
    ...
    try:
        ...
        metadata_rows = [
            ...
            ("Scope", attrs.get("lmm_scope_label", "")),
            ("Fit status", attrs.get("lmm_fit_status_label", "")),
            ("Fit status detail", attrs.get("lmm_fit_status_message", "")),
            ("Fixed-effects contract", attrs.get("lmm_fixed_effects_summary", "")),
            ("Random-effects contract", attrs.get("lmm_random_effects_summary", "")),
            ("Coding summary", attrs.get("lmm_coding_summary", "")),
