...
def _load_rois() -> Dict[str, List[str]]:
    mgr = SettingsManager()
...
ROIS: Dict[str, List[str]] = _load_rois()

def set_rois(rois_dict: Dict[str, List[str]]) -> None:
    """Update the module-level ROI dictionary from user settings."""
    global ROIS
...
# Frequency helpers
# -----------------------------------------------------------------------------
def get_included_freqs(base_freq: float, all_col_names, log_func, max_freq: Optional[float] = None) -> List[float]:
    """
...
# -----------------------------------------------------------------------------
def _ttest_onesample_tail(x: np.ndarray, popmean: float = 0.0, tail: str = "greater", min_n: int = 3):
        ...
        for roi_name in ROIS.keys():
            ...
            for (freq_val, harm_k) in oddball_list:
                ...
                for pid in subjects:
                    ...
                    # Mean over ROI channels for this participant & harmonic
                    mean_val = df_cur.reindex(roi_channels)[col_name].dropna().mean()
                ...
                if n_subj < min_subjects:
                    roi_records.append({
                        'Condition': cond_name, 'ROI': roi_name,
                        'Frequency': display_col, 'Frequency_Hz': float(freq_val), 'Harmonic_k': int(harm_k),
                        'N_Subjects': n_subj, 'mean': np.nan, 't_stat': np.nan, 'p_raw': np.nan,
                        'p_corr': np.nan, 'Significant': False, 'cohens_dz': np.nan,
                        'ci95_low': np.nan, 'ci95_high': np.nan, 'shapiro_p': np.nan,
                        'wilcoxon_p': np.nan
                    })
                    continue

                # One-sample t (tail-aware)
                t_stat, p_raw = _ttest_onesample_tail(np.asarray(subj_vals), popmean=0.0, tail=tail, min_n=min_subjects)
                ...
                if do_wilcoxon_sensitivity and n_subj >= min_subjects:
                    try:
                        ...
                        _, p_w = stats.wilcoxon(np.asarray(subj_vals), zero_method="wilcox", correction=False, alternative=alt)
                    except TypeError:
                        # Fallback for very old SciPy: two-sided then halve if mean>0
                        _, p_two = stats.wilcoxon(np.asarray(subj_vals), zero_method="wilcox", correction=False)
                ...
                roi_records.append({
                    'Condition': cond_name, 'ROI': roi_name,
                    'Frequency': display_col, 'Frequency_Hz': float(freq_val), 'Harmonic_k': int(harm_k),
                    'N_Subjects': n_subj, 'mean': m, 't_stat': t_stat, 'p_raw': p_raw,
                    'cohens_dz': dz, 'ci95_low': ci_l, 'ci95_high': ci_h,
                    'shapiro_p': sh_p, 'wilcoxon_p': p_w
                })
            ...
            if sig_rows:
                ...
                for r in sig_rows:
                    ...
                    p_str = "< .0001" if (r['p_raw'] is not None and np.isfinite(r['p_raw']) and r['p_raw'] < 1e-4) else f"{r['p_raw']:.4f}"
                    pc_str = "< .0001" if (r['p_corr'] is not None and np.isfinite(r['p_corr']) and r['p_corr'] < 1e-4) else f"{r['p_corr']:.4f}"
                    output_lines.append("    -------------------------------------------")
                    output_lines.append(f"    Harmonic: {r['Frequency']} (k={r['Harmonic_k']}) -> SIGNIFICANT RESPONSE")
                    output_lines.append(f"        Average {selected_metric}: {r['mean']:.3f} (N={r['N_Subjects']})  "
                                        f"dz={r['cohens_dz']:.2f}  95% CI [{r['ci95_low']:.3f}, {r['ci95_high']:.3f}]")
                    output_lines.append(f"        t({r['N_Subjects']-1}) = {r['t_stat']:.2f},  p_raw = {p_str},  p_{correction_method} = {pc_str}")
                    if do_wilcoxon_sensitivity and np.isfinite(r.get('wilcoxon_p', np.nan)):
                        output_lines.append(f"        Wilcoxon (one-tailed) p = {r['wilcoxon_p']:.4f}  |  Shapiro p = {r['shapiro_p']:.3f} (info)")
                    output_lines.append("    -------------------------------------------")
                if len(sig_rows) > 1:
                    output_lines.append(f"    Summary for {roi_name}: Found {len(sig_rows)} significant harmonics (details above).\n")
            else:
                output_lines.append(f"  --- ROI: {roi_name} ---")
            ...
            for r in roi_records:
                findings.append({
                    ...
                    'Meets_Mean_Threshold': _passes_threshold(r.get('mean', np.nan), mean_value_threshold, tail=tail),
