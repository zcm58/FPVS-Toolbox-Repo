...
class BCAHeatmapTool(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        ...
        options_frame.grid(row=1, column=0, sticky="nsew", padx=PAD_X, pady=PAD_Y)
        ...
        action_frame.grid(row=2, column=0, sticky="ew", padx=PAD_X, pady=PAD_Y)
        ...
        self.status_box.grid(row=3, column=0, sticky="nsew", padx=PAD_X, pady=PAD_Y)
    ...
    def generate_heatmaps(self):
        ...
        for cond in conds:
            self._generate_for_condition(parent_folder, cond)

        messagebox.showinfo("Complete", "Heatmap generation complete.")

    def _generate_for_condition(self, parent_folder: str, condition: str):
        ...
        output_dir = os.path.join(parent_folder, condition, "heatmaps")
        ...
        if min_val > 0:
            min_val = 0.0
        norm = TwoSlopeNorm(vmin=min_val, vcenter=0.0, vmax=vmax)
        ...
        for idx, col in enumerate(freq_names):
            ...
            values = avg_data[:, idx]
            fig, ax = plt.subplots()
            mne.viz.plot_topomap(values, info, axes=ax, cmap=cmap, norm=norm, show=False)
