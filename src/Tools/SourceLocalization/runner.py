...
from . import source_localization, visualization
...
def run_source_localization(
    ...
    brain = None
    if show_brain:
        visualization._ensure_pyvista_backend()
        ...
        try:
            ...
            brain = visualization._plot_with_alpha(
                stc,
                hemi=hemi,
        ...
        visualization._set_brain_alpha(brain, alpha)
        logger.debug("Brain alpha set to %s", alpha)
        visualization._set_brain_title(brain, os.path.basename(stc_path))
        visualization._set_colorbar_label(brain, "Source amplitude")
        try:
            ...
            pass
        visualization.save_brain_screenshots(brain, output_dir)
    if export_rois:
        try:
